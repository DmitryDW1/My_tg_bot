...
"""
Машина состояний (FSM) для загрузки и изменения баннеров
"""
...
async def add_banner2(message: types.Message, state: FSMContext):
    """
    Отслеживание некорректного ввода фото баннера
    """
...
class AddProduct(StatesGroup):
    """
    Состояния для добавления товара в базу данных
    """
    name = State()
    description = State()
    category = State()
...
# Состояние ожидания ввода названия товара
...
# Хендлер отмены и сброса состояния (очередность фильтров)
...
async def back_step_handler(message: types.Message, state: FSMContext) -> None:
    """
    Вернуться к предыдущему шагу или выход по команде "отмена"
    """
    current_state = await state.get_state()
